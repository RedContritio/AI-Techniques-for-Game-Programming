...
//
//-----------------------------------------------------------------------
Controller::Controller(HWND hwndMain): m_NumSweepers(CParam::NumOfSweepers), 
										                     m_pGA(NULL),
										                     m_bFastRender(false),
										                     m_iTicks(0),
										                     m_NumMines(CParam::NumOfMines),
										                     m_hwndMain(hwndMain),
										                     m_iGenerations(0),
                                         cxClient(CParam::WindowWidth),
                                         cyClient(CParam::WindowHeight)
{
  ...
  vector<int> SplitPoints = m_vecSweepers[0].CalculateSplitPoints();
	...
	for (int i=0; i<m_NumSweepers; i++)
		...
		m_vecSweepers[i].PutWeights(m_vecThePopulation[i].m_weights );

	//initialize mines in random positions within the application window
	for (int i=0; i<m_NumMines; ++i)
	{
		m_vecMines.push_back(S2DVector(RandFloat() * cxClient,
                                   RandFloat() * cyClient));
	}
...
//	method.
//-------------------------------------------------------------------
void Controller::WorldTransform(vector<SPoint> &VBuffer, S2DVector vPos)
{
	//create the world transformation matrix
	C2DMATRIX::TransformSPoints(VBuffer ,vPos.x ,vPos.y ,CParam::MineScale ,CParam::MineScale ,0 );
}
...
{
	//run the sweepers through CParam::NumOfTicks amount of cycles. During
  //this loop each sweepers NN is constantly updated with the appropriate
  //information from its surroundings. The output from the NN is obtained
  //and the sweeper is moved. If it encounters a mine its fitness is
  //updated appropriately,
	if (m_iTicks++ < CParam::NumOfTicks )
	{
		for (int i=0; i<m_NumSweepers; ++i)
		{
			...
			if (GrabHit >= 0)
      {
        ...
        //mine found so replace the mine with another at a random 
			  //position
			  m_vecMines[GrabHit] = S2DVector(RandFloat() * cxClient,
                                        RandFloat() * cyClient);
      }

			//update the chromos fitness score
			m_vecThePopulation[i].m_Fitness = m_vecSweepers[i].Fitness();
...
void Controller::PlotStats(HDC surface)
{
    ...
    //render the graph
    double HSlice = 1.0*cxClient/(m_iGenerations+1);
    double VSlice = 1.0*cyClient/((m_pGA->BestFitness()+1)*2);
