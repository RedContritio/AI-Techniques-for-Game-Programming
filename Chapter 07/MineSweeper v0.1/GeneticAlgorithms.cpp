...
void GeneticPopulation::Crossover(const std::vector<double> &parent1, const std::vector<double> &parent2, std::vector<double> &child1, std::vector<double> &child2)
{
	...
	if ( RandFloat() < m_crossoverRate )
	{
		...
		int p2 = RandInt(p1, parent1.size());
		for ( int i = p1; i<=p2; ++i )
		{
...
std::vector<GENOME> GeneticPopulation::Epoch(std::vector<GENOME> &prev_pop)
{
	m_genes = prev_pop;
	...
	{
		GrabNBest(prev_pop, NewGeneration, Params::NumElite, Params::NumCopiesElite);
	}
