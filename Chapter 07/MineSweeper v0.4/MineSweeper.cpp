...
m_fitness(), m_closestMine(0), m_deadtimes(0), m_deltafitness(2)
{
...
m_position(), m_lookat(), m_rotation(), m_speed(), m_lTrack(MaxTrackSpeed), m_rTrack(MaxTrackSpeed),
m_fitness(), m_closestMine(0), m_deadtimes(0), m_deltafitness(1)
{
	m_position = GetRandomVector2d();
}
...
bool MineSweeper::Update(const std::vector<RedContritio::Vector2d> &mines, const std::vector<SweeperKiller> &killers)
{
	if ( m_deadtimes >= 5 ) return true;

	UpdateClosestMine(mines);

	std::vector<double> inputs;
	Vector2d targetDirection(Normalize(mines[m_closestMine] - m_position));
	Vector2d EnemyDirection(killers[m_closestKiller].Position() - m_position);
	...
	double DisEnemy = EnemyDirection.length();
	double angleEnemy = acos(fabs(EnemyDirection * m_lookat) / fabs(DisEnemy)) * sign(m_lookat, targetDirection);
	inputs.push_back(DisEnemy);
	inputs.push_back(angleEnemy);
	...
	m_speed = Clamp(m_speed, -Params::MaxSpeed, Params::MaxSpeed);
...
void MineSweeper::Reset(int code)
{
	m_position = GetRandomVector2d();
	m_rotation = RandFloat(0, 2*Params::PI);
	m_deltafitness *= 0.8;
...
void MineSweeper::UpdateClosestKiller(const std::vector<SweeperKiller> &killers)
{
	m_closestKiller = 0;
	Vector2d world(Params::WindowWidth, Params::WindowHeight);
	double dis = 1.0 * world.length();
	for ( unsigned i = 0; i<killers.size(); ++i )
	{
		Vector2d vec(VectorDifferenceInWorld(m_position, killers[i].Position(), world));
		if ( vec.length() < dis )
		{
			m_closestKiller = i;
			dis = vec.length();
