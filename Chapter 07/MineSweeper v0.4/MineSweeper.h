...
private:
	...
	double m_fitness, m_deltafitness;
	int m_closestMine;
	int m_closestKiller;
	int m_deadtimes;
	void UpdateClosestMine(const std::vector<RedContritio::Vector2d> &mines);
	void UpdateClosestKiller(const std::vector<SweeperKiller> &killers);
public:
	MineSweeper(void);
	MineSweeper(const RedContritio::NeuralNetwork::NeuralNet &brain);
	~MineSweeper(void);
	bool Update(const std::vector<RedContritio::Vector2d> &mines, const std::vector<SweeperKiller> &killers);
	void Render(HDC surface) const;
	void Reset(void);
	void Reset(int code); // if dead
